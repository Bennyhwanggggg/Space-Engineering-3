%% function 'Kerplerian Orbit model'
%
% Plots the orbit of satellite in ECI frame
%
% Input  :   inclination, right_asc, eccentricity, omega,
%            mean_anomaly, mean_motion, time stamp
% Outputs:  satellite eci position
%
% Kuan Chun Hwang

%% This orbital calculation function will plot 3D orbit plot and ground tracking plot
function sat_eci_pos = kepl_orbit(inclination, right_asc, eccentricity, omega, mean_anomaly, mean_motion,t_stamp)
...
for t=0:t_stamp:86400
    ...
    % Find True Anomaly
    true_anomaly_rad = 2 * atan2(sqrt((1+eccentricity)/(1-eccentricity)) * tan (essentric_anomaly_ans/2),1);
    ...
    sat_eciPos(1,position_number) = satellite_eci_pos(1);
    sat_eciPos(2,position_number) = satellite_eci_pos(2);
    sat_eciPos(3,position_number) = satellite_eci_pos(3);
